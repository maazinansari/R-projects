...
So, the value of the hundred year flood is `r trunc(v_mle)` $\text{ft}^3/\text{s}$ according to maximum likelihood estimation and `r trunc(v_mom)` $\text{ft}^3/\text{s}$ accordint to method of moments estimation. From the data, we see that in only one year, 1955, was the value exceeded.
